        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                    @can('manager_registration_abstract-submission-create')
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}"
                           class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
                    @endcan
                    @can('manager_registration_abstract-submission-activity-log-trash')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": "{{route('manager.get.abstract-submission')}}",
                    "data": function (d) {
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.get.abstract-submission-status', [':sid',':uid',':id']) }}";
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            var output = "";
                            var abstract = '{{ route("manager.abstract-file", ":id") }}';
                            abstract = abstract.replace(':id', data.id);
                            output += '<a href="' + abstract + '" class="btn btn-sm bg-primary"><img src="{{asset('manager/coreui/icons/eye.png')}}" width="20" height="20"></a>';
                            return output;
                        }
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            ...
                            let URL = "{{ route('manager.session.user-registration.abstract-submission.show', [':sid',':uid',':id']) }}";
                            URL = URL.replace(':id', row.id);
                            URL = URL.replace(':sid', {{$sdata}});
                            URL = URL.replace(':uid', {{$udata}});

                            let ACTIVITY = "{{ route('manager.get.abstract-submission-activity', [':sid',':uid',':id']) }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);
                            ACTIVITY = ACTIVITY.replace(':sid', {{$sdata}});
                            ACTIVITY = ACTIVITY.replace(':uid', {{$udata}});

                            return '<div class="d-flex">' +
                                @can('manager_registration_abstract-submission-show')
                                    '<a class="me-1" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_registration_abstract-submission-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_registration_abstract-submission-activity-log')
                                    '<a class="me-1" href="' + ACTIVITY + '"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_registration_abstract-submission-delete')
