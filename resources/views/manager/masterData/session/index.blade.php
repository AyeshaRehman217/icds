            ...
            $('#indextable').DataTable({
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.get.session-status', ':id') }}";
                            URL = URL.replace(':id', row.id);
                            var output = "";
                            @can('manager_master-data_session-status-edit')
                            if (data == 1) {
                                output += '<a href="' + URL + '"><span class="badge bg-success">Active</span></a>';
                            } else {
                                output += '<a href="' + URL + '"><span class="badge bg-danger">In-Active</span></a>';
                            }
                            @endcan
                                return output;
                        }
                    },
                    {
                        ...
                        orderable: false,
                        render: function (data, type, row, meta) {
                            let URL = "{{ route('manager.session.show', ':id') }}";
                            URL = URL.replace(':id', row.id);

                            let USER_REGISTRATION = "{{ route('manager.session.user-registration.index',':sid') }}";
                            USER_REGISTRATION = USER_REGISTRATION.replace(':sid', row.id);

                            let ACTIVITY = "{{ route('manager.get.session-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);

                            return '<div class="d-flex">' +
                                @can('manager_master-data_session-show')
                                    '<a class="me-1" href="' + URL + '"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('manager_master-data_session-edit')
                                    '<a class="me-1" href="' + URL + '/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('manager_master-data_session-activity-log')
                                    '<a class="me-1" href="' + ACTIVITY + '"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('manager_master-data_session-delete')
                                ...
                                @endcan
                                    ...
                                    // '<a class="me-1" href="' + USER_REGISTRATION + '"><span class="badge bg-primary text-light">More</span>' +
                                '<a class="ms-1" href="' + USER_REGISTRATION + '" data-coreui-toggle="tooltip" data-coreui-placement="top" title="User Registration"> <span class="badge bg-primary text-light">More<i class="fa fa-forward ms-2"></i></span> </a>'+
                                @endcan
                                    '</div>'
