        ...
        <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-toolbar d-none d-md-block" role="toolbar" aria-label="Toolbar with buttons">
                   {{-- {{ logger('Test') }} --}}
                    @if(!isset($abstract))
                    @can('user_registration_abstract-submission-create')
                        <a href="{{(!empty($url) && isset($url)) ? $url : ''}}" class="btn btn-sm btn-primary">{{(!empty($url_text) && isset($url_text)) ? $url_text : ''}}</a>
                    @endcan
                    @endif
                    @can('user_registration_abstract-submission-activity-log-trash')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#indextable').DataTable({
                ...
                ajax: {
                    "type":"GET",
                    "url":"{{route('user.get.abstract-submission')}}",
                    "data": function(d) {
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            var output = "";
                            var abstract = '{{ route("user.abstract-file", ":id") }}';
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row, meta ) {
                            ...
                            let URL = "{{ route('user.abstract-submission.show', ':id') }}";
                            URL = URL.replace(':id', row.id);


                            let ACTIVITY = "{{ route('user.get.abstract-submission-activity', ':id') }}";
                            ACTIVITY = ACTIVITY.replace(':id', row.id);


                            return '<div class="d-flex">' +
                                @can('user_registration_abstract-submission-show')
                                    '<a class="me-1" href="'+URL+'"><span class="badge bg-success text-dark">Show</span>' +
                                @endcan
                                    @can('user_registration_abstract-submission-edit')
                                    '<a class="me-1" href="'+URL+'/edit"><span class="badge bg-info text-dark">Edit</span></a>' +
                                @endcan
                                    @can('user_registration_abstract-submission-activity-log')
                                    '<a class="me-1" href="'+ACTIVITY+'"><span class="badge bg-warning text-dark">Activity</span></a>' +
                                @endcan
                                    @can('user_registration_abstract-submission-delete')
